    ...
    <div class="profile-card">
      ...
      <div class="avatar-section">
        <div class="avatar-container">
          <img v-if="imagePreview" :src="imagePreview" alt="用户头像" class="avatar-image" />
        ...
        <div class="avatar-actions">
          ...
          <button class="album-btn" @click="goToAlbum">我的相册</button>
        </div>
        <input 
          type="file" 
          id="photo-upload" 
      ...
      <div class="form-container">
        ...
        <div class="form-group">
          ...
          <select id="education" v-model="userInfo.education">
        ...
        <div class="form-group introduction-group">
          ...
          <div class="rich-editor-container">
            <div class="editor-toolbar">
              <button type="button" class="toolbar-btn" @click="applyFormat('bold')" title="加粗">
                <span class="icon">B</span>
              </button>
              <button type="button" class="toolbar-btn" @click="applyFormat('italic')" title="斜体">
                <span class="icon">I</span>
              </button>
              <button type="button" class="toolbar-btn" @click="applyFormat('underline')" title="下划线">
                <span class="icon">U</span>
              </button>
              <div class="toolbar-divider"></div>
              <div class="emoji-picker">
                <button type="button" class="toolbar-btn emoji-btn" @click="toggleEmojiPicker" title="表情符号">
                  <span class="icon">😊</span>
                </button>
                <div class="emoji-panel" v-if="showEmojiPicker">
                  <div class="emoji-list">
                    <span v-for="emoji in emojiList" :key="emoji" class="emoji" @click="insertEmoji(emoji)">{{ emoji }}</span>
                  </div>
                </div>
              </div>
            </div>
            <textarea 
              ...
              ref="introTextarea"
            ></textarea>
            <div class="char-counter" :class="{ 'limit-near': charCount > 400, 'limit-reached': charCount >= 500 }">
              {{ charCount }}/500
            </div>
          </div>
        </div>
  ...
  methods: {
    ...
    async saveUserInfo() {
      try {
        ...
        // 发送请求
        const response = await axios.post(`http://localhost:8080/api/users/profile`, formData, {
          headers: {
    ...
    insertEmoji(emoji) {
      ...
      // 在光标位置插入表情
      this.userInfo.introduction = text.substring(0, start) + emoji + text.substring(end);
    ...
    applyFormat(format) {
      ...
      // 检查插入后是否超过字符限制
      const newLength = this.userInfo.introduction.length - selectedText.length + formattedText.length;
      if (newLength > 500) {
        return;
      }
      
      // 替换选中的文本
      this.userInfo.introduction = this.userInfo.introduction.substring(0, start) + formattedText + this.userInfo.introduction.substring(end);
