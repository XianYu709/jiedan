...
class Track {
  ...
  _onPostRender({ viewer, viewOption }) {
    ...
    if (Cesium.JulianDate.lessThanOrEquals(now, this._endTime)) {
      ...
      // 方向
      const orientation = this._velocityOrientation.getValue(now)
      if (orientation) {
      ...
      const time = this._timeLine[this._positionIndex]
      if (time) {
        const timeDiff = Cesium.JulianDate.secondsDifference(now, time)
        if (timeDiff >= 0 && timeDiff <= 1) {
          const position = this._positions[this._positionIndex] || undefined
          if (position && orientation) {
            const mat = Cesium.Matrix3.fromQuaternion(orientation)
            const mat4 = Cesium.Matrix4.fromRotationTranslation(mat, p)
            const hpr = Cesium.Transforms.fixedFrameToHeadingPitchRoll(mat4)
  ...
  _setCameraView(viewer, viewOption) {
    ...
    if (Cesium.JulianDate.greaterThan(now, this._endTime)) {
      ...
      const p = this._sampledPosition.getValue(now)
      const next_p = this._sampledPosition.getValue(
  ...
  _resetTimeLine(params) {
    ...
    if (!params?.stopTime && !params?.duration) {
      const v = distance(this._positions) / this._duration
   ...
   * @returns {Track}
   */
  addPosition(position, duration = 20) {
