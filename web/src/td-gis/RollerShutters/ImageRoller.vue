<template>
  <div class="flex flex-col">
    ...
    <List
      class="mt-4 bg-white overflow-y-auto h-30"
      size="small"
      bordered
...
const setRollerShutterSplit = (scene, rollerShutterConfig) => {
  ...
  // 参与卷帘的影像图层
  let imageryLayer = viewer.imageryLayers._layers;
  for (let i = 0, j = imageryLayer.length; i < j; i++) {
    if (isRoller.value && imageryLayerList.value[i].checked) {
      imageryLayer[i].swipeEnabled = true;
      imageryLayer[i].swipeRegion = scratchSwipeRegions;
    } else {
