<template>
  <Dialog v-model:open="open" noPadding width="300px" @cancel="handleCancel" title="图层控制">
    <Tabs style="margin-left: 20px" defaultActiveKey="1">
      <TabPane
        v-for="(tabName, tabKey) in ['基础地图', '应急资源', '风险区域']"
        :tab="tabName"
        :key="tabKey"
      >
        <div class="p-2">
          <list :data-source="listData" size="small">
            <template #renderItem="{ item, index }">
              <list-item v-if="item.type === tabKey + 1">
                <Checkbox v-model:checked="item.checked" @click="(e) => handleClick(item, e)"
                >{{ item.name }}
                </Checkbox>
              </list-item>
            </template>
  ...
  <Dialog
    class="top-50 right-15 absolute"
    v-model:open="openBestPath"
    :width="380"
    :title="`路线规划:  起点(${startPointInfo.name})`"
    @cancel="bpHandleCancel"
  >
    <View @select="selectGoal" :params="startPointInfo" @avoid="setAvoid"/>
  </Dialog>
</template>
...
const openBestPath = ref(false);
const bpHandleCancel = () => {
  openBestPath.value = false;
};
watch(
  () => props,
  () => {
...
const handleClick = (item, e) => {
  ...
  switch (item.name) {
    ...
    case '路网':
      ...
      break;
    case '医院':
      hanlderEmergency(item.name, 'http://101.43.167.87:9900/yy.png', 30, 30);
      break;
    case '防空洞':
      hanlderEmergency(item.name, 'http://101.43.167.87:9900/fkd.png', 30, 30);
      break;
    case '避难所':
      hanlderEmergency(item.name, 'http://101.43.167.87:9900/bns.png', 30, 30);
      break;
    case '消防站':
      hanlderEmergency(item.name, 'http://101.43.167.87:9900/xfz.png', 30, 30);
      break;
    case '洪水':
                  ...
                  marker.on('click', async () => {
                    ...
                    const infoWindow = new AMap.InfoWindow({
                      content: `<div class="custom-info-window">
                                    ...
                                    <p>电话：${poi.tel || '暂无'}</p>
                                    <p>容量：${
                        data.records.length !== 0 ? data.records[0].capacity : '暂无'
                      }</p>
                                  </div>`,
...
const emergencyMap = {};
// 危险区
const startPointInfo = ref<any>({});

const clearEmergency = (type?: string) => {
      ...
      if (item.checked) {
        ...
        data.records.forEach((item, index) => {
          ...
          polygon.on('click', async (e) => {
            openBestPath.value = true;
            startPointInfo.value = {lnglat: e.lnglat, name: item.name};
          });
          map.add(polygon);
          ...
          map.add(text);
          emergencyMap[`${type}${index}-text`] = text;
        });
...
let setAvoid = (e) => {
  driving && driving.clear();
  driving && driving.setAvoidPolygons([JSON.parse(e)]);
  driving && driving.search(...paramsDriving);
};

const selectGoal = (res) => {
  AMap.plugin(['AMap.Driving'], function () {
    let selectLocation = JSON.parse(res[0].location);
    driving = new AMap.Driving({
      map: map,
    });
    paramsDriving = [
      startPointInfo.value.lnglat,
      new AMap.LngLat(selectLocation[0], selectLocation[1]),
    ];
    driving.search(...paramsDriving);
  });
};
</script>
